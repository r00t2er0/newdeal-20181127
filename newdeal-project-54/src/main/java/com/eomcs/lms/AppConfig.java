...
// 인터페이스의 메서드 파라미터는 SQL 매퍼의 파라미터 타입과 같아야 한다.
// 인터페이스의 메서드 이름 파라미터와 리턴 타입이 SQL 매퍼의 파라미터 타입, 리턴 타입과 같아야 한다.
@MapperScan("com.eomcs.lms.dao")
public class AppConfig {
  
  // Spring IoC 컨테이너가 로딩한 프로퍼티 정보를 가져오기
  // 로딩한 프로퍼티 값 중에서 jdbc.driver라는 이름을 가진 값을 꺼내 jdbc.driver변수에 넣음
  @Value("${jdbc.driver}")
  String jdbcDriver;
...
//트랜젝션 객체를 생성할 때 기본 이름으로 transactionManager라고 설정하라
 // 다른 이름으로 설정하면 트래젝션 관련하여 다른 객체 생성할 때 
 ...
 public PlatformTransactionManager transactionManager(DataSource dataSource) {
  ...
  public SqlSessionFactory sqlSessionFactory(
    ...
    // Domain 클래스(VO) 별명 자동 생성하기
    factoryBean.setTypeAliasesPackage("com.eomcs.lms.domain");
    
    // SQL 매퍼 파일 로딩
    // SQL 매퍼 파일에 위치 정보를 Resource 객체에 담아서 넘겨야 한다.
    ...
    factoryBean.setMapperLocations(iocContainer.getResources(
        "classpath:/com/eomcs/lms/conf/mapper/*Mapper.xml"));
